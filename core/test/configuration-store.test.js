...
const fetchImpl = async (url, options = {}) => {
  ...
  if (url.includes("$filter=Ativo eq 1")) return response(200, { value: [item] });
  if (url.includes("/items?") && url.includes("$orderby=Title")) return response(200, { value: [item] });
...
assert.equal((await store.list("forum"))[0].id, 4);
