...
function topicFilter({ view, categoryId, search }) {
  ...
  else if (view === "resolved") filters.push("(Status eq 'Resolvido' or Status eq 'Encerrado')");
...
function displayAuthorName(item) {
  const legacyName = typeof item?.LegacyCreatedByName === "string" ? item.LegacyCreatedByName.trim() : "";
  return legacyName || item?.Author?.Title || "Autor não informado";
}

function displayAuthorKey(item) {
  const legacyEmail = typeof item?.LegacyCreatedByEmail === "string" ? item.LegacyCreatedByEmail.trim().toLowerCase() : "";
  const legacyName = typeof item?.LegacyCreatedByName === "string" ? item.LegacyCreatedByName.trim() : "";
  if (legacyEmail || legacyName) return `legacy:${legacyEmail || legacyName.toLowerCase()}`;
  if (item?.Author?.Id) return `sp:${item.Author.Id}`;
  return null;
}

function displayCreatedAt(item) {
  return item?.LegacyCreatedAt || item?.Created || null;
}

function withDisplayMeta(item) {
  return {
    ...item,
    DisplayAuthor: displayAuthorName(item),
    DisplayCreatedAt: displayCreatedAt(item)
  };
}
...
export function createForumReadService({ dataSources, sanitizeRichText } = {}) {
  ...
  function isOwnersGroup(group) {
    const title = String(group?.Title ?? group?.title ?? "").trim();
    return /\bowners\b/i.test(title) || /propriet[aá]rios/i.test(title);
  }
  ...
  async function currentUser() {
    currentUserPromise ??= (async () => {
      const response = await dataSources.getClient("forum-topics").request(
        "/_api/web/currentuser?$select=Id,Title,IsSiteAdmin,Groups/Title&$expand=Groups"
      );
      const data = response.data?.d ?? response.data;
      const groups = data?.Groups?.results ?? data?.Groups?.value ?? data?.Groups ?? [];
      return freezeResult({
        id: positiveInteger(data?.Id, "currentUser.Id"),
        title: data?.Title || "Autor não informado",
        isSiteOwner: Boolean(data?.IsSiteAdmin) || (Array.isArray(groups) && groups.some(isOwnersGroup))
      });
    })().catch((error) => {
  ...
  async function listContributors({ limit = 5 } = {}) {
    ...
    const bump = (item, changes) => {
      const id = displayAuthorKey(item);
      if (!id) return;
      const current = contributors.get(id) ?? {
        userId: id,
        name: displayAuthorName(item),
        topics: 0,
        answers: 0,
        points: 0,
        score: 0
    ...
    const [topics, answers] = await Promise.all([
      dataSources.getClient("forum-topics").getListItems(topicSource, {
        select: ["Id", "Status", "QuantidadeRespostas", "Pontuacao", "IsMigrated", "LegacyCreatedByName", "LegacyCreatedByEmail", "Author/Id", "Author/Title"],
        expand: "Author",
      ...
      dataSources.getClient("forum-answers").getListItems(answerSource, {
        select: ["Id", "Status", "Pontuacao", "IsMigrated", "LegacyCreatedByName", "LegacyCreatedByEmail", "Author/Id", "Author/Title"],
        expand: "Author",
  ...
  async function getTopicForClose(topicId) {
    const id = positiveInteger(topicId, "topicId");
    const source = dataSources.get("forum-topics");
    const [loaded, user] = await Promise.all([
      dataSources.getClient("forum-topics").getListItem(source, id, {
        select: ["Id", "Status", "Author/Id"],
        expand: "Author"
      }),
      currentUser()
    ]);
    ...
    return freezeResult({ topic: loaded.item, etag: loaded.etag });
  }
  ...
  async function listAnswers(topicId, { pageSize = 30, cursor } = {}) {
    ...
    return freezeResult({
      answers: page.items.map((item) => ({ ...withDisplayMeta(item), canEdit: item.Author?.Id === userId })),
      next: page.next
    });
  }
  ...
  async function listCategorySummaries({ recentLimit = 3 } = {}) {
    ...
    const topics = await dataSources.getClient("forum-topics").getListItems(source, {
      select: ["Id", "Title", "CategoriaId", "IsMigrated", "LegacyCreatedAt", "LegacyCreatedByName", "LegacyCreatedByEmail", "Author/Id", "Author/Title", "UltimaAtividade"],
      expand: "Author",
      filter: "Status ne 'Arquivado'",
      orderBy: "UltimaAtividade desc,Id desc",
      top: 500
    ...
    return freezeResult(categories.map((category) => {
      ...
      return {
        ...
        recentTopics: items.slice(0, recentLimit).map((topic) => ({
          id: topic.Id,
          title: topic.Title,
          author: displayAuthorName(topic),
  ...
  async function listForumOverview({ unansweredDays = 3 } = {}) {
    ...
    const [topics, answers, relations] = await Promise.all([
      dataSources.getClient("forum-topics").getListItems(topicSource, {
        select: ["Id", "CategoriaId", "Status", "Fixado", "QuantidadeRespostas", "UltimaAtividade", "IsMigrated", "LegacyCreatedByName", "LegacyCreatedByEmail", "Author/Id"],
        expand: "Author",
        filter: "Status ne 'Arquivado'",
        orderBy: "Id desc",
        top: 500
      ...
      dataSources.getClient("forum-answers").getListItems(answerSource, {
        select: ["Id", "IsMigrated", "LegacyCreatedByName", "LegacyCreatedByEmail", "Author/Id"],
        expand: "Author",
        filter: "Status eq 'Publicada'",
        orderBy: "Id desc",
        top: 500
    ...
    for (const topic of topics) {
      ...
      if (topic.Status === "Resolvido" || topic.Status === "Encerrado") resolved += 1;
    ...
    for (const answer of answers) {
      const key = displayAuthorKey(answer);
      if (key) activeAuthors.add(key);
    }
