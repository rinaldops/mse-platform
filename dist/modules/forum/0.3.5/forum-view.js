...
function displayAuthor(item) {
  return item?.DisplayAuthor || item?.Author?.Title || "Autor não informado";
}

function displayCreatedAt(item) {
  return item?.DisplayCreatedAt || item?.Created || item?.Modified || item?.UltimaAtividade;
}
...
// Icon-only action (edit/close/archive-answer/mark solution/permalink): the visible label
...
export function createForumView({
  ...
  function topicCard(topic) {
    ...
    const author = displayAuthor(topic);
    meta.textContent = `${author} · ${formattedDate(displayCreatedAt(topic))}`;
    main.append(meta);
  ...
  function topicListCard(topic) {
    ...
    const avatar = element(document, "div", "mse-forum__topic-avatar", initials(displayAuthor(topic)));
    ...
    body.append(element(
      ...
      `${author} · ${formattedDate(displayCreatedAt(topic))}`
    ));
    ...
    if (topic.Status === "Encerrado") {
      badges.append(element(document, "span", "mse-forum__topic-badge mse-forum__topic-badge--closed", "Encerrado"));
  ...
  async function renderDetail(route, sequence) {
    ...
    try {
      ...
      topicActions.append(element(
        ...
        "span",
        "mse-forum__meta",
        `${displayAuthor(topic)} · ${formattedDate(displayCreatedAt(topic))}`
      ));
      ...
      const drawAnswer = (answer) => {
        ...
        const metaLine = element(document, "span", "mse-forum__meta", `${displayAuthor(answer)} · ${formattedDate(displayCreatedAt(answer))}`);
      ...
      content.replaceChildren(
        ...
        ...(topic.Status === "Arquivado" || topic.Status === "Fechado" || topic.Status === "Encerrado" ? [] : [reply]),
        related
      );
