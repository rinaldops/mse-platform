...
export function videoPlayerUrl(video) {
  ...
  if (fileRef.startsWith("/")) return `/_layouts/15/stream.aspx?id=${encodeURIComponent(fileRef)}`;
  return video?.URL || "#";
}
...
export function createVideotecaView({ root, service, reducedMotion, storage = globalThis.localStorage } = {}) {
  ...
  function configureVideoLink(link, video) {
    ...
    link.addEventListener("click", () => handleOpen(video));
    return link;
  }
  ...
  async function render() {
    ...
    const requestedId = Number(new URLSearchParams(document.defaultView?.location?.search || "").get("video"));
    const requested = Number.isInteger(requestedId) ? allVideos.find((video) => video.Id === requestedId) : null;
