...
export function createVideotecaView({ root, service, reducedMotion, storage = globalThis.localStorage } = {}) {
  ...
  function stopActivePlayback(expectedPlayer = null) {
    if (!activePlayback || (expectedPlayer && activePlayback.player !== expectedPlayer)) return;
    const { container, player, originalContent } = activePlayback;
    activePlayback = null;
    player.pause?.();
    player.removeAttribute?.("src");
    player.load?.();
    container.classList.remove("mse-videoteca__media--playing");
    container.replaceChildren(...originalContent);
    carousel?.start?.();
  }

  function addMediaLauncher(container, video, durationClass = null) {
    const launch = element(document, "button", "mse-videoteca__media-launch");
    launch.type = "button";
    launch.setAttribute("aria-label", `Assistir ${video.Title}`);
    launch.addEventListener("click", () => playInline(container, video));
    if (durationClass && video.Duracao) {
      launch.append(element(document, "span", durationClass, video.Duracao));
    }
  ...
  function playInline(container, video) {
    if (activePlayback?.container === container) return;
    stopActivePlayback();
    handleOpen(video);
    ...
    const stop = element(document, "button", "mse-videoteca__player-stop", "■ Parar");
    stop.type = "button";
    stop.setAttribute("aria-label", `Parar ${video.Title}`);
    stop.addEventListener("click", () => stopActivePlayback(player));
    player.addEventListener("ended", () => stopActivePlayback(player));
    container.replaceChildren(player, stop);
    container.classList.add("mse-videoteca__media--playing");
    activePlayback = { container, player, originalContent };
    carousel?.stop?.();
    player.play?.().catch?.(() => {});
  }
  ...
  function thumb(video) {
    ...
    addMediaLauncher(node, video, "mse-videoteca__duration");
    return node;
  }
  ...
  function renderCarousel(featured) {
    ...
    return { element: section, dispose: built.destroy, start: built.start, stop: built.stop };
  }
  ...
  function recentList(videos) {
    ...
    for (const video of videos.slice(0, SHOWCASE_LIMIT)) {
      ...
      const image = element(document, "div", "mse-videoteca__latest-thumb");
      const imageUrl = thumbnailFor(video);
      if (imageUrl) image.style.backgroundImage = `url('${imageUrl}')`;
      addMediaLauncher(image, video);
...
export function createVideotecaSummaryView({ root, service, pageHref, limit = 8 } = {}) {
  ...
  let visibleLimit = summaryColumnCount(root.getBoundingClientRect?.().width, limit);
  let resizeObserver = null;

  function visibleVideos() {
    if (activeCategory === null) {
      const all = catalog.groups.flatMap((group) => group.videos);
      return (catalog.featured.length ? catalog.featured : all).slice(0, visibleLimit);
    }
    const group = catalog.groups.find((entry) => entry.category === activeCategory);
    return (group?.videos ?? []).slice(0, visibleLimit);
  }
  ...
  function renderShell() {
    ...
    list.style.setProperty("--summary-columns", String(Math.max(1, videos.length)));
    ...
    try {
      ...
      if (typeof ResizeObserverClass === "function") {
        resizeObserver = new ResizeObserverClass(([entry]) => {
          const nextLimit = summaryColumnCount(entry?.contentRect?.width, limit);
          if (!disposed && nextLimit !== visibleLimit) {
