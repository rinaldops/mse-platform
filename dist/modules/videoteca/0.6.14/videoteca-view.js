...
export function categoriesFor(video) {
  const value = video?.Categorias?.results ?? video?.Categorias;
  const categories = Array.isArray(value) ? value : [];
  const normalized = categories.map((category) => String(category).trim()).filter(Boolean);
  if (!normalized.length && video?.Categoria) normalized.push(String(video.Categoria).trim());
...
export function createVideotecaSummaryView({ root, service, pageHref, limit = 8 } = {}) {
  ...
  function visibleVideos() {
    if (activeCategory === null) {
      ...
      const featuredIds = new Set(catalog.featured.map((video) => video.Id));
      return [...catalog.featured, ...all.filter((video) => !featuredIds.has(video.Id))].slice(0, visibleLimit);
    }
  ...
  function renderShell() {
    ...
    list.style.setProperty("--summary-columns", String(Math.max(1, Math.min(visibleColumns, videos.length))));
