...
export function createVideotecaView({ root, service, reducedMotion, storage = globalThis.localStorage } = {}) {
  ...
  function recentList(videos) {
    ...
    for (const video of videos.slice(0, SHOWCASE_LIMIT)) {
      const link = element(document, "a", "mse-videoteca__latest-item");
      link.href = video.URL;
      link.target = "_blank";
      link.rel = "noopener noreferrer";
      link.addEventListener("click", () => handleOpen(video));
      const image = element(document, "span", "mse-videoteca__latest-thumb");
      ...
      text.append(element(document, "span", "mse-videoteca__latest-name", video.Title));
      const date = formattedDate(video.Data);
