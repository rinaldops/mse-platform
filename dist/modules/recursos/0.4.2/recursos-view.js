...
export function createRecursosView({ root, service } = {}) {
  ...
  function pageBar() {
    ...
    crumb.append(element(document, "h1", "mse-recursos__pagebar-title", "Explore"));
    inner.append(crumb);
    ...
    search.type = "search";
    search.placeholder = "Buscar no Explore...";
    search.setAttribute("aria-label", "Buscar no Explore");
...
export function createRecursosSummaryView({ root, service, pageHref, limit = 6 } = {}) {
  ...
  function renderShell() {
    ...
    header.append(element(document, "h2", "mse-recursos__summary-title", "Explore"));
    const cta = element(document, "a", "mse-recursos__summary-cta", "Ver Explore completo");
    cta.href = pageHref;
    header.append(cta);
    panel.append(header);
    ...
    list.replaceChildren(...(links.length
      ? links.map(linkCard)
      : [element(document, "p", "mse-recursos__summary-empty", "Nenhum link nesta categoria.")]));
    panel.append(list);
